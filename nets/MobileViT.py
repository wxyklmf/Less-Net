...
from .mobilevit_utils.transformer import TransformerEncoder
from .mobilevit_utils.mdoel_config import get_config
...
class InvertedResidual(nn.Module):
    ...
    def __init__(
        ...
        hidden_dim = make_divisible(
            v=int(round(in_channels * expand_ratio)), divisor=8
        )  # 调整通道数为8的倍数
...
class MobileViTBlock(nn.Module):
    ...
    def unfolding(self, x: Tensor) -> Tuple[Tensor, Dict]:
        ...
        batch_size, in_channels, orig_h, orig_w = x.shape  # (bs, 64, 28, 28)

        new_h = int(math.ceil(orig_h / self.patch_h) * self.patch_h)  # 28
        new_w = int(math.ceil(orig_w / self.patch_w) * self.patch_w)  # 28
        # 选择线性插值的方式对图片的尺寸进行调整（特征图与期望尺寸不一致时）
        interpolate = False
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        fm = self.local_rep(x)  # (bs, 48, 28, 28)

        # convert feature map to patches
        patches, info_dict = self.unfolding(fm)  # patches(64, 196, 64)
...
class MobileViT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.conv_1(x)  # x(bs, 3, H, W) -> x(bs, 16, H/2, W/2)
        x1 = self.layer_1(x)  # x(bs, 32, H/2, W/2)
        x2 = self.layer_2(x1)  # x(bs, 64, H/4, W/4)
        h2, w2 = x2.shape[2], x2.shape[3]  # H/4, W/4
        x3 = self.layer_3(x2)  # x(bs, 96, H/8, W/8)
        h3, w3 = x3.shape[2], x3.shape[3]  # H/8, W/8
        x4 = self.layer_4(x3)  # x(bs, 128, H/16, W/16)
        x5 = self.layer_5(x4)  # x(bs, 160, H/32, W/32)
        # x = self.conv_1x1_exp(x)  # x(bs, 640, H/32, W/32)
        # x = self.classifier(x)  # x(bs, num_classes)
...
def mobile_vit_xx_small(num_classes: int = 1000):
    ...
    m = MobileViT(model_cfg=config, num_classes=num_classes)
    m = MobileViT(model_cfg=config, num_classes=num_classes)
    pretrained_weights = torch.load("mobilevit_xxs.pt")
...
def mobile_vit_small(num_classes: int = 1000):
    ...
    m = MobileViT(model_cfg=config, num_classes=num_classes)
    pretrained_weights = torch.load("mobilevit_s.pt")
    m.load_state_dict(pretrained_weights)
    print("成功导入small")
    # 记得加预训练模型权重
    return m

def mobile_vit_small_backbone(model_type="small"):
    backbone = None
    if model_type == "small":
        backbone = mobile_vit_small()
    elif model_type == "x_small":
        backbone = mobile_vit_x_small()
    elif model_type == "xx_small":
        backbone = mobile_vit_xx_small()
    return backbone

if __name__ == '__main__':
    model = mobile_vit_small()
    random_input = torch.randn(1, 3, 640, 640)
    x1, x2, x3, x4, x5 = model(random_input)
    print(x1.shape)
    print(x2.shape)
    print(x3.shape)
    print(x4.shape)
