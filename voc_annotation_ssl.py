...
if __name__ == "__main__":
    ...
    unlabeled_filepath = os.path.join(VOCdevkit_path, 'VOC2007/Unlabeled')
    temp_unlabeled = os.listdir(unlabeled_filepath)
    total_unlabeled = [x for x in temp_unlabeled if x.endswith(".jpg")]

    num = len(total_unlabeled)
    list = range(num)
    tv = int(num)
    tr = int(tv)
    ...
    print("train size", tr)
    funlabeled = open(os.path.join(saveBasePath, 'unlabeled.txt'), 'w')

    for i in list:
        name = total_unlabeled[i][:-4] + '\n'
        funlabeled.write(name)

    funlabeled.close()
    print("Generate txt for Unlabeled in ImageSets done.")
